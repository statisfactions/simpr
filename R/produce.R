...
#'
#' @param spec a \code{simpr_meta} object generated by \code{\link{meta}},
#'  containing the specifications of the simulation
...
produce = function(spec, reps) {
  ...
  ## get meta info
  if("simpr_spec" %in% class(spec) ) {
    ## this is if meta() was never called
    specs = data.frame(rep = 1:reps)
    x = spec
  } else {
    x = list(meta = attr(spec, "meta"),
           variables = attr(spec, "variables"))

    specs = dplyr::inner_join(data.frame(rep = 1:reps), spec, by = character())
  }
