#' Fit models to the simulated data in parallel
#'
#' Takes simulated data from \code{\link{produce}} and applies functions to it,
#' usually model-fitting functions using parallel computation.
#'
...
#'   future_fit(linear = ~lm(y ~ x1, data = .))
#'
#' linear_fit
#'
...
#'   glance_fits
#'
#' ## We can also use .$colname syntax in future_fit(), e.g.,:
#' linear_fit = simple_linear_data %>%
#'   future_fit(linear = ~lm(.$y ~ .$x1)) # no need for "data=."
#'
...
#'   future_fit(add_five = ~ tibble::as_tibble(. + 5))
#'
#' add_five_data
#'
