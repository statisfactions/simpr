---
title: "Exploring and Debugging Simulation Errors"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{simulation-errors}
...
Quite often, errors will arise in the course of simulating or fitting models.  These errors can be tricky to debug because they may happen within a simulation function and only in certain cases.  Consider the following example:
...
The default behavior is to warn when there are errors, and storing those error messages in a separate directory.  This gives you the flexibility to continue with the simulation or model-fitting process even if one particular simulation hit an error.
...
Even in this degenerate case, `tidy_fits` still gathers together whatever valid fits it can find, and ignores the rest:
